...
import {createValidatorSchema} from "../lib/utils.ts";
import {User} from "../models/user.ts";
import {signupSchema, loginSchema, type SignupType, type LoginType} from "../schemas/auth-schema.ts";
...
authRoute.post(
    ...
    async (c) => {
        try {
            ...
            const findUser = await User.exists({username: body.username});
...
authRoute.post(
    "/signup",
    createValidatorSchema(signupSchema),
    async (c) => {
        try {
...
authRoute.post(
    "/login",
    createValidatorSchema(loginSchema),
    async (c) => {
        try {
